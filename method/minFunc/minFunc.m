...
global TrainLabel;
global TestLabel;
global visibleSize;                % input data feature dimensions
global hiddenSize;                                 % number of hidden units 
global lambda;                                 % weight decay
global alpha; 
global beta; 
global gamma; 
global sourceSize;
...
for i = 1:maxIter
    ...
    if verboseI
        ...
        %{
        fprintf('\n');
        W1 = reshape(x(1:hiddenSize*visibleSize), hiddenSize, visibleSize);
        b1 = x(2*hiddenSize*visibleSize+1:2*hiddenSize*visibleSize+hiddenSize);
        C = reshape(x(2*hiddenSize*visibleSize+hiddenSize+visibleSize+1:end), sourceSize, hiddenSize);
        result = test(TrainData, TestData, TrainLabel, TestLabel, W1, b1, C);
        fprintf('the accuracy of train data is:  %f       %f        %f\n', result(1,1), result(1,2), result(1,3));
        fprintf('the accuracy of test data is:  %f     %f     %f     %f\n', result(1,4), result(1,5), result(1,6), result(1,7));
        %}
    end
    ...
    %test(TrainData, TestData, TrainLabel, TestLabel, W1, b1, C);
