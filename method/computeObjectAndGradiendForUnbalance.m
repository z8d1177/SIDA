function [object,grad] = computeObjectAndGradiendForUnbalance(theta, option_unbalance, traindata, testdata, trainlabel)

    % parameter init
    numM = option_unbalance.numM;
    numK = option_unbalance.numK;
    numC = option_unbalance.numC;
    numX = option_unbalance.numX;
    alpha = option_unbalance.alpha;
    beta = option_unbalance.beta;
    gamma = option_unbalance.gamma;
    ...
    % Calculate J1  %求解第一项, unbalance_CE
    ...
    p1 = ee./total(:,1:1:numX(1,1));
    J1 = J1 + sum((1-p1)*log(p1));
    ee = exp(W2(2,:)*hiddenvalues(:,numX(1,1)+1:1:numX(1,1)+numX(1,2)) + b2(2,:) * weightsbuffer(:,numX(1,1)+1:1:numX(1,1)+numX(1,2)));
    p2 = ee./total(:,numX(1,1)+1:1:numX(1,1)+numX(1,2));
    J1 = J1 + sum((1-p2)*log(p2));

    % Calculate J2  %求解第二项, MMD
...
%     J2 = sum(Ps.*log(Ps./Pt)) + sum(Pt.*log(Pt./Ps));
    phi_s = sum(hiddenvalues(:,1:1:traindatasize), 2)./traindatasize;
    phi_t = sum(hiddenvalues(:,traindatasize+1:1:datasize), 2)./testsize;
    J2 = phi_s - phi_t;
%     fprintf('J2: %d\n',J2);
    
    % Calculate J3  %求解第三项，含Label项
%     J3=0;
%     total = sum(exp(labelinputs(:,1:1:traindatasize)));
%     ee = exp(W2(1,:)*hiddenvalues(:,1:1:numX(1,1)) + b2(1,:)*weightsbuffer(:,1:1:numX(1,1)));
...
%     fprintf('object: %d\n',object);   
    ...
    W1grad2 = W1grad2 + DD(:,1:1:traindatasize).*(ones(1,traindatasize))*data(:,1:1:traindatasize)'/traindatasize;
    W1grad2 = W1grad2 + DD(:,traindatasize+1:1:datasize).*(ones(1,testsize))*data(:,traindatasize+1:1:datasize)'/testsize;
    b1grad2 = b1grad2 + DD(:,1:1:traindatasize)*ones(traindatasize,1)/traindatasize;
    b1grad2 = b1grad2 + DD(:,traindatasize+1:1:datasize)*ones(testsize,1)/testsize;
...
%     b1grad3 = b1grad3 + 2*W2'*((labelvalues(:,1:1:traindatasize)-[trainlabel;ones(1,traindatasize)-trainlabel]).*CC(:,1:1:traindatasize)).*DD(:,1:1:traindatasize)*ones(traindatasize,1);
    ...
    W1grad = W1grad1 + alpha * W1grad2 +  2 * gamma * W1;   
    b1grad = b1grad1 + alpha * b1grad2 +  2 * gamma * b1; 
